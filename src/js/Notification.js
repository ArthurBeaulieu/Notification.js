/**
 * This file has no license, it is creative common, if you want to tweak it, use it, make money with it, share it, feel free to go :) -- Arthur
 * @version 1.0
 *
 * @summary Notification handler for web applications
 * @description Automate the handling of app notifications to the minimum code to trigger - KISS
 * @classdesc Notification.js is part of the MessCss web framework. It can both work stand alone and in MessCss. The class itself is pretty easy to use and can be set up in a minute, but also gives the possibility to tweak it a lot, using options. -- KISS (Keep It Simple, Stupid!)<br><br>There are four different types of notifications to give your users different feedbacks: Succes, Info, Warning and Error. These are highly customizable using options object. Also, the CSS style is made in <code>SCSS</code>, so you can tweak the variable to make this class perfectly fit your needs.
 *
 * @file Notification.js
 * @author Arthur Beaulieu
 * @since June 2018
 *
 * @example <caption><hr><br><h2>Notification handler</h2>The easiest way to implement this class is to attach an instance to the window DOM object (1), so you can access it from anywhere in your app.<br>Optionnaly, you can pass an options object to tune the handler behavior (2).<br><b>Warning</b>: transition time must not exceed half the duration (for obvious, logical questions), otherwise, it will be resetted to the default value.</caption>
 * // (1) - Default options (see class documentation)
 *window.notification = new Notification();
 *
 * // (2) - Custom notification handler
 *window.notification = new Notification({
 *    position:    'top-right', // Notification handler position in viewpoint // TODO add top-center, bottom center
 *    thickBorder: 'top',      // Global notification thick border location
 *    duration:    5000,        // Total duration of a notification on screen in milliseconds
 *    transition:  200,         // Fade in and out transition duration in milliseconds
 *    maxActive:   10            // The max number of simultaneously opened notification
 *});
 * @example <caption><hr><br><h2>Notification type</h2>Once attached to the DOM, add a notification from anywhere in your app logic (3).<br>You can also call a new typed notification (4) if you find it more suitable for your need.<br><b>Note</b>: <code>type</code> and <code>message</code> are the only mandatory options (no need to put type when using example (3), since they are already typed).<br>Also, storing the notification ID is only useful if you intend to manually dismiss the notification later on.</caption>
 * // (3) - Simple notification type abstraction
 *let ids = window.notification.success({ message: 'Great success!' });
 *let idi = window.notification.info({ message: 'Much info, very text' });
 *let idw = window.notification.warning({ message: 'Snake?' });
 *let ide = window.notification.error({ message: 'Snaaaaaaaake!' });
 *
 * // (4) - Custom notification construction with all possible parameters
 *let id = window.notification.new({
 *    type:        'info',                         // Mandatory, Notification type
 *    title:       'My test notification',         // Notification head title
 *    message:     'Lorem ipsum dolor sit amet.',  // Mandatory, Notification message
 *	  duration:    5000,						   // Notification duration
 *	  iconless:    false,						   // Icon in the notification
 *    thickBorder: 'top'                           // Local notification thick border location
 *    closable:    true,                           // Ability for user to close notification
 *    sticky:      false,                           // Notification will dim instead of close
 *    renderTo:    document.body,                  // Render the notification anywhere you want
 *    CBtitle:     'My callback',                  // Callback button text
 *    callback:    function() {                    // Callback function
 *        alert('Called from my notification');
 *    }
 *});
 * @example <caption><hr><br><h2>Archived notifications</h2>When a notification reaches the end of its lifecycle and disapear, it is still store sorted in an history array.<br>In this array, you have access to all notification in a chronological order (index 0 is the oldest), their attributes and their timestamps (creation, deletion, duration on screen).</caption>
 *let historyLength = window.notification.getHistoryLength(); // Returns the number of archived notifications
 *let history       = window.notification.getHistory();       // Returns a sorted array of archived notification
 * @example <caption><hr><br><h2>Notification manual destruction</h2>Finally, you can manually dismiss a notification using its ID or dismill them all:</caption>
